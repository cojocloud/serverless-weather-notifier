...
def lambda_handler(event, context):
    ...
    for data in result:
        result_object = {
            ...
            "time": time.strftime(
                "%Y-%m-%d %I:%M %p", time.localtime(data["EpochDateTime"])
            ),
            "icon_phrase": data["IconPhrase"],
            "reception": data["PrecipitationIntensity"]
            + " "
    ...
    result = ses_client.send_email(
        ...
        Message={
            "Body": {"Html": {"Charset": "UTF-8", "Data": render(final_result),}},
            "Subject": {"Charset": "UTF-8", "Data": "Today's Weather",},
        },
        Source=EMAIL_SOURCE,
    )
